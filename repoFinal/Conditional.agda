...
-- El condicional se forma a partir de una flecha p: A →  Bool la cual nos permite definir la siguiente flecha:
 ...
 rightDistr p f g h = proof p ⇒ f , g ∙ h
                      ...
                      [ f , g ] ∙ copair unit unit ∙ distr ∙ ⟨ iden ∙ h , p ∙ h ⟩
                            ...
                            ≅⟨ congr (congr (congr (cong₂ ⟨_,_⟩ (symIden h) (sym idl)))) ⟩  -- Damos vuelta y hacemos aparecer iden
                      ...
                      (distr ∙ pair h (copair iden iden)) ∙ ⟨ iden , p ∙ h ⟩
                            ...
                            ≅⟨ congr (congr (congl (natDistr h iden iden))) ⟩  -- Usamos la natDistr que decia 
                                                                               -- distr ∙ (pair f (copair g h)) ≅ (copair (pair f g) (pair f h)) ∙ distr
                      [ f , g ] ∙
                      copair unit unit ∙
...
-- Este lemma nos va a servir para la proxima propiedad
 ...
 lemma {A} {B} {D} {X} {Z} f g h  = proof pair f [ g , h ]
                                    ...
                                    ∙ distr
                                           ...
                                           ≅⟨ congl (cong₂ [_,_] (cong₂ pair idr (Coproducts.law1 hasCoproducts)) 
                                                                (cong₂ pair idr (Coproducts.law2 hasCoproducts))) ⟩
